# !/usr/bin/env python3
import sys, os, math, random, shutil, json, time
import numpy as np
import pandas as pd
...
def update_seeds_LFDR(
    ...
    try:
        ...
        if not distribution or len(z_scores) < min_kde_samples:
...
def main_propagation(argv):
    ...
    parser.add_argument('-data_comp', required=True)
    parser.add_argument('-fp_seed', required=True)
    parser.add_argument('-fp_comp', required=True)
    parser.add_argument('-PT_CB_seed', required=True)
    parser.add_argument('-PT_CB_comp', required=True)
    ...
    DATA_SEED['id'] = DATA_SEED['id'].str.replace('t','p')
    DATA_COMP = pd.read_csv(args.data_comp, sep=',')
    FP_SEED = pd.read_pickle(args.fp_seed)
    FP_COMP = pd.read_pickle(args.fp_comp)
    
    PT_CB_SEED = pd.read_pickle(args.PT_CB_seed)
    PT_CB_COMP = pd.read_pickle(args.PT_CB_comp)
    ...
    data_comp = FP_COMP[keyword]
    seed_ids = DATA_SEED['id'].tolist()
    comp_ids = DATA_COMP['id'].tolist()
    
    TOTAL_NODES = seed_ids + comp_ids
    PT_CB_combine = np.vstack([PT_CB_SEED, PT_CB_COMP])
    PT_CB_DICT = {node: PT_CB_combine[i] for i, node in enumerate(TOTAL_NODES)}
    pt_cb_dim = PT_CB_combine.shape[1]
    
    FP_combine = np.vstack([FP_SEED[keyword], FP_COMP[keyword]])
    ...
    def precompute_pca_similarity_parzen(
        ...
        data_combined = np.vstack([data_seed, data_comp])
        node_ids = seed_ids + comp_ids
    ...
    for sample_idx in tqdm(range(max_samples), desc="Processing train/val splits"):
        ...
        try:
            ...
            node2sim = precompute_pca_similarity_parzen(data_seed, data_comp, seed_ids, comp_ids, train_seeds, n_components=2, gamma=1, aggregator='mean')
            ...
            while train_count < MAX_ITER and patience > 0:
                ...
                efs_current = calculate_enrichment_factor(node_scores, val_seeds, [0.005])
